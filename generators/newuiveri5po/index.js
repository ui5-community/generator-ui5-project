    ...
    prompting() {
        return this.prompt([
            ...
            function (answers) {
                ...
                this.options.oneTimeConfig.poFile =
                    this.options.oneTimeConfig.poName.charAt(0).toLowerCase() +
    ...
    writing() {
        ...
        this.fs.copyTpl(
            ...
            this.destinationPath(
                this.options.oneTimeConfig.dirname + "pages/" + this.options.oneTimeConfig.poFile + ".js"
            ),
        ...
        specs.forEach((spec) => {
            ...
            if (fs.existsSync(specFile)) {
                ...
                content =
                    `require("./pages/${this.options.oneTimeConfig.poFile}");\n` +
                    content.substr(0, content.lastIndexOf("});"));
                Object.keys(this.options.oneTimeConfig.uiveri5pos).forEach(
                    function (poFile) {
                        const poName = poFile.charAt(0).toUpperCase() + poFile.substr(1);
                        content +=
                            `  it("should see the ${poName} page", function () {\n    // call the page object's actions and assertions:\n` +
